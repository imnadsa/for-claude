    ...
    return (
      <div className="mb-4 px-4">
        <div className="flex justify-between text-xs mb-1">
          <span className="text-slate-400">Осталось бесплатных запросов: {messagesLeft} из {messagesLimit}</span>
          {percentLeft < 30 && (
            <button className="text-teal-400 hover:text-teal-300 hover:underline">
              Перейти на Pro
            </button>
          )}
        </div>
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-120px)] bg-slate-950 border border-slate-800 rounded-lg overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-900/50">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold font-fixedsys">
            {chatId ? 'Продолжение диалога' : 'Новый чат'}
          </h2>
          {isPro && (
            ...
            {messages.map((message, index) => (
              ...
              >
                ...
                >
                  ...
                  <div className={`flex mt-2 pt-2 text-xs ${
                    message.sender === 'user' ? 'justify-end border-t border-indigo-800/30' : 'justify-between border-t border-slate-800'
                  }`}>
                    {message.sender === 'ai' && (
                      <div className="flex items-center gap-2">
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full text-slate-500 hover:text-teal-400">
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full text-slate-500 hover:text-red-400">
